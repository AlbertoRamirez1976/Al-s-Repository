...
/* Verify theta the string ends with the suffix provided, but is not
 * a copy of the suffix */
int stringEndsWith(const char *s, const char *suffix)
{
...
/* is_valid_int checks that str is a valid (signed) int */
...
/* loadCodeFromFile loads an sml program directly into MEMORY */
void loadCodeFromFile()
{
    int lineNumber, opCode, validLine;
    int sourceLineNumber = 0;             /* Use for debugging purposes */
    char line[ 100 ];
    char *linePtr;
    ...
    if(!stringEndsWith(filename, ".sml")) {
